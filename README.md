...
- **간편 로그인** — Chrome 쿠키 자동 추출로 별도 설정 없이 인증 (macOS)
...
```json
{
  "mcpServers": {
    "velog": {
      "command": "node",
      "args": ["/path/to/velog-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add velog node /path/to/velog-mcp/dist/index.js
```

### 환경변수로 인증 (선택)

토큰을 직접 설정할 수도 있습니다. 자동 로그인 기능을 사용하지 않는 경우나 macOS 외 환경에서 사용할 때 유용합니다.
...
3. `access_token`과 `refresh_token` 값 복사

> 읽기 전용으로 사용할 경우 인증 없이도 작동합니다.
...
1. 환경변수 (`VELOG_ACCESS_TOKEN`, `VELOG_REFRESH_TOKEN`)
2. 저장된 파일 (`~/.velog-mcp/tokens.json`)
3. 미인증 (읽기 전용)
...
`access_token`이 만료되면 `login` 도구를 다시 호출해주세요.
